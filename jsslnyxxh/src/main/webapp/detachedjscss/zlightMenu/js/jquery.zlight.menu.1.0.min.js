;(function($, window, document, undefined) {
    var pluginName = 'zlightMenu'
      , defaults = {
        ...
        iconColor: '#ffffff',
        iconSize: '20px',
        mobileMainColor: '#29b6f6',
        mainColor: '#29b6f6',
        secondColor: 'rgba(0, 0, 0, .1)',
        fontColor: '#ffffff',
        activeFontColor: '#ffffff',
        borderLightColor: '#4FC3F7',
        borderDarkColor: '#4FC3F7',
        dropBorderLightColor: '#4FC3F7',
        dropBorderDarkColor: '#4FC3F7'
    };
...
//            $(window).on('scroll.ZlightScrolling', function() {
        		...
        		$(".content").mCustomScrollbar({theme:"minimal-dark",scrollInertia:100,scrollButtons:{ enable: true},
        			mouseWheel:{ 
        			    enable: true ,                            //启用或禁用通过鼠标滚轮滚动内容。
        			    scrollAmount: 20                       //滚轮滚动的像素
        			},
        			callbacks:{
        				whileScrolling:function(){
        					if($('.homepageeventloc').length>0){
			        			if ($('#transformTopnavBar').offset().top<0 && ele.counter === 0) {
				                    ele.element.css({
				                        'position': 'fixed',
				                        'top': '-' + ele.mtop + 'px',
				                        'left': '0',
	        			    	...
	        			    	if ($('#transformTopnavBar').offset().top>0 && ele.counter === 1) {
	        	                    ele.element.css({
	        	                        'position': 'relative',
	        	                        'margin': '',
	        	                        'padding': ''
        						...
        						if ($('#transformTopnavBar').offset().top<0) {
	    							$('.to-top').show();
	    						}else{
	    							$('.to-top').hide();
	    						}
