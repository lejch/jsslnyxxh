...
(function($){
  ...
  $.fn.EasyWidgets.defaults = {
    ...
    callbacks : {
      ...
      //当页面被刷新，需要重新初始化widget的折叠或展开状态时
      onRefreshCollapse : null,
  ...
  function InitializeWidgets(
   settings, widgetOnDemand){
     ...
     var collapses = '';
     if($.isFunction(s.callbacks.onRefreshCollapse)){
    	 collapses = s.callbacks.onRefreshCollapse();
     }
     $(s.selectors.widget).each(function(){
       PrepareWidgetBehaviour($(this),b,s,collapses);
     });
   ...
   *
   */
  function PrepareWidgetBehaviour(widget, widgetOnDemand, settings ,collapses){
    var s = settings;
    ...
    if(widgetMenu.html() == null){
      ...
      AddWidgetCollapseLink(widget, widgetMenu, s ,collapses);
  ...
  $.fn.RepositionedWidgets =  function(pos){
	  alert(pos);
	    var positions = pos;
	    if($.trim(positions) != ''){
	      // Get the widgets places IDs and widgets IDs
...
//      sortableItems.css({width:''});
      ...
      if(!widget.hasClass('dragging')){
        widget.css({width:originalWidth});
      }else{
        $(s.selectors.places).sortable('disable');
      }
    });
...
//    $(s.selectors.places).sortable("destroy");
   ...
   *
   */
  function AddWidgetCollapseLink(widget, widgetMenu, settings, collapses){
    var s = settings;
    var link = '';
    var widgetId = widget.attr('id');
    var haveId = $.trim(widgetId) != '';
    ...
    if(widget.hasClass(s.options.collapsable)){
      ...
      if(haveId && $.trim(collapses) != ''){
    	  var subwidgetId = widgetId + '|';
          if(collapses.indexOf(subwidgetId) != -1){
            link = MenuLink(
              s.i18n.extendText,
              s.i18n.extendTitle,
              s.selectors.collapseLink
      ...
      $(link).mousedown(function(e){
        ...
        if(contentVisible){
          ...
          if(canCollapse){
            ...
            if($.isFunction(s.callbacks.onChangeCollapse)){//
              var coll = getCollaspe(s);
              s.callbacks.onChangeCollapse(coll);
            }
  ...
  function getCollaspe(settings){
	  ...
	  $(s.selectors.places).each(function(){
	      ...
	      place.children(s.selectors.widget).each(function(){
	        var wid = $(this);
	        var widId = wid.attr('id');
	        var con = wid.find(s.selectors.content);
	        var collapse = con.css('display') == 'none';
	        var have = $.trim(widId) != '';
	        if(have&&collapse){
	          if(coll == ''){
	        	  coll += widId + '|';
  ...
  function rsetCollaspe(settings,collapses){
	  var s = settings;
	  $(s.selectors.widget).each(function(){
		   var widget = $(this);
		   var widgetId = widget.attr('id');
