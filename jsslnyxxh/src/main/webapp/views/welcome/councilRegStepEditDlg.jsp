...
<body style="overflow:hidden;width:880px;">
<style type="text/css">
...
<script>
	$(document).ready(function(){
		$('#loadInfoStf').html('理事申请时间：<strong>'+parent.BUILDREGDATA.CREATETIME+'</strong> ，已载入申请数据!');
		$('#membRegForm').form('load',parent.BUILDREGDATA);
		
		if(parent.BUILDREGDATA.FLAG!='3'){
			...
			$('#exportAllMemberReg').one('click',function(){
				...
				var form=$("<form>");
				form.attr("style","display:none");form.attr("target","");form.attr("method","post");form.attr("action",appPath+'/hrwp/rexpCouncilReg');
...
<div id="loadInfoStf" class="alert" role="alert" style="padding:10px;margin-top:15px;margin-left:15px;margin-right:15px;width:980px;"></div>
...
<div class="page" id="mrform2">
	...
	<button id='exportAllMemberReg' type="button" class="btn btn-xs expbtn" style="font-size:14px;">重新导出理事申请</button>
</body>
</html>
