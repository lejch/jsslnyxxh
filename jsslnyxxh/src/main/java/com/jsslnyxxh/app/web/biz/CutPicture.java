...
public Object getVerificationCodeCH(HttpServletRequest request, HttpServletResponse response) {
	Random r = new Random();
	int a = r.nextInt(5) + 1;
	String url = request.getSession().getServletContext().getRealPath("/")+"static/images/"+a+".png";
	int x=20,y=20;
	...
	try{
		...
		List<String> unRepeatCheck = new ArrayList<String>();
		
		for (int i = 0; i < 4; i++) { // 4个汉字
			String ch = null;
			do{
				ch = base.charAt(new Random().nextInt(base.length())) + "";
			}while(unRepeatCheck.contains(ch));
			unRepeatCheck.add(ch);
			sb.append(ch);
			...
			if (place == 2) {
				x = new Random().nextInt(60) + 100; // 自己定义的位子坐标
				y = new Random().nextInt(20) + 50; // i=2的时候，y的值
			}
			if (place == 3) {
				x = new Random().nextInt(60) + 100; // 自己定义的位子坐标
				y = new Random().nextInt(60) + 100; // i=3的时候，y的值
			}
			if (place == 4) {
				x = new Random().nextInt(2) + 50; // i=4的时候，x的值
				y = new Random().nextInt(60) + 100; // 自己定义的位子坐标
			}
        ...
        gra.drawString("请在下图按顺序点击：" + sb.toString(), 20
        		, bi.getHeight()/2 + font.getSize()/2);
		...
		try{out.flush();out.close();out = null;}catch(Exception e2){e2.printStackTrace();}
	}
	return null;
}
...
@ResponseBody
public Map<String, Object> verifyCode(HttpServletRequest request,HttpServletResponse response,HttpSession session) {
